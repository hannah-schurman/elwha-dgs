...
for file in model_files:
    ...
    # sediment normalized sand percentage
    sand = percentages.get(0, 0)
    gravel = percentages.get(1, 0)
    cobble = percentages.get(2, 0)
    sediment_total = sand + gravel + cobble

    row["sand_pct_sediment"] = (sand / sediment_total) * 100 if sediment_total > 0 else np.nan
    all_data.append(row)
...
df_update["Filename"] = df_update["Filename"].str.replace("_", "-", regex=False)
...
df_merged = df_excel.merge(
    df_update[["Filename", "class_0", "class_1", "class_2", "class_3", "class_4", "class_5", "sand_pct_sediment"]],
    on="Filename",
    how="left"
)
